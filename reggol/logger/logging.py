...
class Logger(object):
    ...
    def _add_output(self, file_name, arr, fds, mode='a'):
        if file_name not in arr:
            ...
            fds[file_name] = open(file_name, mode, buffering=1)
    ...
    def save_eval_data(self, data, itr):
        ...
        """
        file_name = osp.join(self._snapshot_dir, f"eval_data_{itr}.txt")
        file = open(file_name, 'w')
        for element in data:
            file.write("Start: \n")
            file.write(f"is_success: {element['is_success']}\n")
            file.write(f"episode_length: {element['episode_length']}\n")
            file.write(f"# of objects: {element['num_of_objects']}\n")
            file.write(f"objects: {element['objects']}\n")
            file.write(f"goal: {element['goal']}\n")
